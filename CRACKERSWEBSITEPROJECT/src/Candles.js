...
import { Container, Row, Col, Button, Card, Breadcrumb, Modal } from 'react-bootstrap';
import axios from 'axios';

function Products({ addItemToCart }) { // Ensure addItemToCart is destructured correctly
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      const response = await axios.post('/api/purchases', {
        ...
        itemId: `item_${modalContent}`, 
        totalPrice: basePrices[modalContent] * counts[modalContent]
      });

      console.log('Status:your order is confirmed:', response.data);
      setOrderConfirmed(true);
      setSaveMessage(null);
    } catch (error) {
      if (error.response) {
        console.error('Error response:', error.response.data);
        setSaveMessage(`Status: ${error.response.data.message || 'your order is not confirmed.'}`);
  ...
  const handleSaveClick = async (index) => {
    const itemData = {
      itemImage: `https://www.kausikcrackers.com/image/cache/catalog/novelties/${[
        'wonders%20(1)',
        'scooby',
        'mayajal',
        'paris',
  ...
  return (
    <Container style={{ padding: '20px' }}>
      <Breadcrumb>
        ...
        <Breadcrumb.Item active>Candles</Breadcrumb.Item>
      </Breadcrumb>
          ...
          <Col md={3} key={index}>
            ...
            >
              <Card style={{ width: '100%' }}>
                <Card.Img                                                                                            
                  variant="top"
                  src={`https://www.kausikcrackers.com/image/cache/catalog/novelties/${index === 0 ? 'wonders%20(1)' : index === 1 ? 'scooby' : index === 2 ? 'mayajal' : index === 3 ? 'paris' : 'Untitled%20design'}-228x228.png `}
